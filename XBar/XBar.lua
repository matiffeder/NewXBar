...
_G.XBar = XBar
_G.XBARVERSION=1.71 --will deleted
local pStart=1
local pMax=20
local xwindow
...
local function MoveVar()
	...
	XBarFrame:SetWidth(GetScreenWidth())
	for _,v in ipairs(XAddons.btn) do
		local s = XBSet[v.."_X"]
		local b = _G["XBar"..v]
		...
		if b then
			...
			if XBSet["Position"]=="TOP" then
				...
				b:SetAnchor("LEFT","LEFT","XBarFrame",s*w,-2)
			else
...
local function BarVar()
	if XBSet["Enable"] then
		...
		if XBSet["Position"]=="TOP" then
			...
			XBarFrame:SetAnchor("TOP","TOP","UIParent",0,0)
			...
			XBarFrame:ClearAllAnchors()
			XBarFrame:SetAnchor("BOTTOM","BOTTOM","UIParent",0,0)
		...
		for i,v in ipairs(XAddons.btn) do
			local b = _G["XBar"..v]
		...
		for i,v in ipairs(XAddons.btn) do local b = _G["XBar"..v] if b and i~=1 then b:Hide() _G["XBar"..v.."_BG"]:Hide() end end
	end
end
...
function XBar.Default()
	for k,v in pairs(def) do XBSet[k]=v end
...
function XBar.BarDesign_OnShow()
	for _,v in ipairs(xdsgn) do UIDropDownMenu_AddButton({text=v,func=DesignOnClick}) end
end
...
function XBar.AM_OnShow(this)
	...
	for _,v in ipairs(XAddons.gui) do if v.open then v.open=false end end
...
function XBar.AM_ListBtn_OnClick(this)
	...
	for _,v in ipairs(XAddons.gui) do
		if v.open then
	...
	for i,v in ipairs(XAddons.btn) do if not _G["XBar"..v] and i<14 and i~=1 then _G["XBarCfg_B_"..v]:Hide() end end
end
...
function XBar.AM_ListBtn_Update(nowid)
	...
	for i,v in ipairs(XAddons.gui) do
...
function XAddon_Register(add) --XBar.AM_Register
	if add.gui then
		for _, v in ipairs(add.gui) do
			assert(v.window, "\n|cffFFE855XBar:|r Third-party addon's |cffFFE855gui.window|r is required.")
	...
	if add.popup then
		for _, v in ipairs(add.popup) do
			assert(v.GetText, "\n|cffFFE855XBar:|r Third-party addon's |cffFFE855popup.GetText|r is required.")
			table.insert(XAddons.popup, v)
		end
	end
	if add.btn then
		local n = assert(add.btn.name, "\n|cffFFE855XBar:|r Third-party addon's |cffFFE855btn.name|r is required.")
		local m = "XBar"..n
		local bg = "XBar"..n.."_BG"
		local bt = "XBar"..n.."_B"
		local fn = "XBar"..n.."_F"
		local lm = _G[m]
		if not pcall(function() _G[m]:Show() end) then
			lm = CreateUIComponent("Frame", m, "UIParent")
			lm:SetSize(128, 32)
			lm:SetFrameStrata("BACKGROUND")
			lm:SetScripts("OnShow", [=[_G[this:GetName().."_BG"]:Show()]=])
			lm:SetScripts("OnHide", [=[_G[this:GetName().."_BG"]:Hide()]=])
		end
		local lbg = CreateUIComponent("Frame", bg, "XBarFrame")
		lbg:SetSize(128, 32)
		lbg:ClearAllAnchors()
		lbg:SetAnchor("LEFT", "LEFT", lm, -28, 0)
		local lbgt = CreateUIComponent("Texture", "XBar"..n.."_BG_T", bg)
		lbgt:SetSize(128, 32)
		lbg:SetLayers(1, lbgt)
		local lbt = CreateUIComponent("Button", bt, m)
		lbt:SetSize(20, 20)
		lbt:ClearAllAnchors()
		lbt:SetAnchor("LEFT", "LEFT", lm, 0, 0)
		local lbtt = CreateUIComponent("Texture", "XBar"..n.."_B_T", bt)
		lbtt:SetSize(20, 20)
		lbtt:SetTexture(add.btn.texture or "Interface/Icons/drug_001")
		...
		lfn:SetSize(512, 24)
		lfn:ClearAllAnchors()
		lfn:SetAnchor("LEFT", "LEFT", lm, 26, 0)
		local lfns = CreateUIComponent("FontString", "XBar"..n.."_F_S", fn)
		lfns:SetSize(512, 24)
		lfns:SetFont("Fonts/DFHEIMDU.TTF", 12, "NORMAL", "NORMAL")
		lfns:SetJustifyHType("LEFT")
		lfn:SetLayers(1, lfns)
		table.insert(XAddons.btn, n)
		MoveVar()
	end
end
...
function XBar.PopupMenu_OnShow()
	...
	for i,v in ipairs(XBar_PopupMenu.Buttons) do
